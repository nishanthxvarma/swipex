...
Extracts contact info, categorized skills, education, work experience, projects, certifications,
...
SECTION_PATTERNS: Dict[str, str] = {
    "summary": r'(?i)^(summary|professional summary|profile|about me|objective|executive summary|overview)\b',
    "experience": r'(?i)^(work experience|professional experience|experience|employment history|work history|career history|internships)\b',
    "education": r'(?i)^(education|academic background|academics|qualifications|educational background|degrees)\b',
    "skills": r'(?i)^(skills|technical skills|core competencies|technologies|expertise|skills & tools|skillset|tech stack)\b',
    "projects": r'(?i)^(projects|personal projects|technical projects|academic projects|key projects|featured projects)\b',
    "certifications": r'(?i)^(certifications|licenses|credentials|certificates|certifications & licenses)\b',
    "achievements": r'(?i)^(achievements|awards|honors|accomplishments|publications|hackathons)\b',
    "languages": r'(?i)^(languages|spoken languages|language proficiency)\b',
}
...
class ResumeParser:
    ...
    def parse(self, file_bytes: bytes, file_type: str, filename: str) -> Dict[str, Any]:
        ...
        projects_list, proj_spans = self._extract_projects(proj_text)
        ...
        return {
            ...
            "_metadata": {
                ...
                "extraction_confidence": doc.get("confidence", 0.90),
    ...
    def _segment_sections(self, lines: List[str]) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if len(line_clean.split()) <= 5 and not re.search(r'[:@|\d{4}]', line_clean):
    ...
    def _extract_contact_info(self, lines: List[str], raw_text: str, filename: str) -> Tuple[Dict[str, str], Dict[str, Any]]:
        ...
        # Portfolio URL
        portfolio_match = re.search(r'(?:https?://)?(?:www\.)?([A-Za-z0-9_-]+\.(?:io|dev|me|app|site|tech))', raw_text, re.IGNORECASE)
        if portfolio_match:
        ...
        # Location heuristic: City, ST / City, Country
        loc_match = re.search(r'\b([A-Z][a-zA-Z\s]+,\s*(?:[A-Z]{2}|[A-Z][a-zA-Z\s]+))\b', raw_text[:500])
        if loc_match and not any(k in loc_match.group(1).lower() for k in ("university", "college", "engineer", "developer", "resume")):
            info["location"] = loc_match.group(1).strip()
            spans["location"] = {"value": info["location"], "confidence": 0.80}

        # Candidate Name extraction from first valid non-header lines
        candidate_name = ""
        for line in lines[:5]:
            # Discard lines with emails, links, or numbers
            if re.search(r'[@/\\|]|http|\.com|\.io|\d{3}', line):
                continue
            words = line.split()
            if 1 <= len(words) <= 4 and re.match(r'^[A-Za-z\s\.\'-]+$', line):
                # Ensure it's not a common section title
                if line.lower() not in ("curriculum vitae", "resume", "cv", "summary", "profile", "contact"):
                    candidate_name = line.strip()
                    break

        if not candidate_name and filename:
            clean_name = re.sub(r'[-_]', ' ', filename.split('.')[0])
            clean_name = re.sub(r'(?i)(resume|cv|latest|updated|final|v\d+)', '', clean_name).strip().title()
            if clean_name and len(clean_name.split()) <= 4:
                candidate_name = clean_name

        info["name"] = candidate_name or "Applicant"
        spans["name"] = {"value": info["name"], "confidence": 0.90 if candidate_name else 0.40}

        # Professional headline heuristic from first 6 lines
        for line in lines[1:6]:
            if re.search(r'(?i)\b(engineer|developer|architect|designer|manager|scientist|specialist|consultant|analyst|lead)\b', line):
    ...
    def _extract_skills(self, skills_text: str, full_text: str) -> Tuple[Dict[str, List[str]], Dict[str, Any]]:
        ...
        # If section extraction yielded very few items, run on full document
        total_found = sum(len(items) for items in extracted_taxonomy.values())
        if total_found < 5:
    ...
    def _extract_education(self, edu_text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        Extracts education entries without hallucinating fake universities.
        """
        if not edu_text:
            return [], []
        ...
        for line in lines:
            ...
            # Check for GPA
            gpa_match = re.search(r'(?i)\b(?:gpa|cgpa)[:\s]*([0-9]\.[0-9]{1,2}(?:\s*/\s*[0-9](?:\.0)?)?)', line)

            # Check for College / University name
            is_school = re.search(r'(?i)\b(university|college|institute|school|academy|polytechnic)\b', line)

            if is_school or degree_found:
                if current_entry and (current_entry.get("college") or current_entry.get("degree")):
                    entries.append(current_entry)
                    spans.append({"entry_id": current_entry["id"], "evidence": current_entry.get("degree", "") + " at " + current_entry.get("college", "")})
                
                current_entry = {
                    "id": f"edu_{len(entries) + 1}",
                    "degree": degree_found or "Degree",
                    "college": line.split(",")[0].strip() if is_school else "Institution",
                    "cgpa": gpa_match.group(1) if gpa_match else "",
                    "graduationYear": year_match.group(0) if year_match else ""
                }
            elif current_entry:
                if not current_entry.get("cgpa") and gpa_match:
                    current_entry["cgpa"] = gpa_match.group(1)
                if not current_entry.get("graduationYear") and year_match:
                    current_entry["graduationYear"] = year_match.group(0)

        if current_entry and (current_entry.get("college") or current_entry.get("degree")):
            entries.append(current_entry)
            spans.append({"entry_id": current_entry["id"], "evidence": current_entry.get("degree", "")})

        return entries, spans

    def _extract_experience(self, exp_text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        Extracts work experience entries grounded strictly in document text.
        """
        if not exp_text:
            return [], []
        ...
        for line in lines:
            # Detect title / role / company boundary
            is_date_line = re.search(date_range_pattern, line)
            is_job_title = re.search(r'(?i)\b(engineer|developer|architect|designer|lead|manager|intern|specialist|analyst|director|vp|head)\b', line)

            if is_job_title and (is_date_line or len(line.split()) <= 8):
                if current_exp:
                ...
                # Check if date was part of this line
                date_str = ""
    ...
    def _extract_projects(self, proj_text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for line in lines:
            # Check if this line looks like a project title (short, contains pipe or tech bracket)
            is_title = (len(line.split()) <= 7 and not line.startswith("-") and not line.startswith("•")) or "|" in line
            if is_title and not re.search(r'(?i)\b(responsibilities|summary|description)\b', line):
                if current_proj:
    ...
    def _extract_certs_from_text(self, text: str) -> List[str]:
        ...
        cert_keywords = [
            ...
            r'(?i)\b(Google Cloud Certified [A-Za-z\s-]+)\b',
    ...
    def _extract_languages_from_text(self, text: str) -> List[str]:
        langs = []
        common_langs = ["English", "Spanish", "French", "German", "Mandarin", "Hindi", "Japanese", "Arabic", "Portuguese", "Russian"]
